...
// upon `bar` function call create and push new execution context
...
callStack.push(barExecutionContext);

// assign bar a value and store in current execution context
// `bar` returns so remove current execution context
callStack.shift();
...
Whenever a function returns but contains a closure its execution context cannot just be removed from the call stack as there may still be a need to access any variables it references, so it's stored instead on what is known as the "heap" - a dynamically allocated piece of memory - until the function's inner variables have been resolved for the final time. 
